    ...
    public partial class MatchZy
    {
        ...
        public void LoadMatch(CCSPlayerController? player, CommandInfo command)
        {
            try
            {
                ...
                if (isMatchSetup)
                {
                    command.ReplyToCommand($"[LoadMatch] 一场比赛已经设置好了id: {liveMatchId}, 不能加载新的比赛!");
                ...
                {
                    command.ReplyToCommand($"[LoadMatch] 提供的文件不存在! 请使用: matchzy_loadmatch <文件名>");
                ...
                if (!success)
                {
                    command.ReplyToCommand("比赛加载失败! 请重新设置当前的比赛！");
                    ResetMatch();
                }
            }
            catch (Exception e)
        ...
        public void LoadMatchFromURL(CCSPlayerController? player, CommandInfo command)
        {
            ...
            if (isMatchSetup)
            {
                command.ReplyToCommand($"[LoadMatchDataCommand] 一场比赛已经设置好了id: {liveMatchId}, 不能加载新的比赛!");
            ...
            if (!IsValidUrl(url))
            {
                command.ReplyToCommand($"[LoadMatchDataCommand] 无效的 URL: {url}. 请提供有效的URL来加载比赛!");
            ...
            try
            {
                ...
                {
                    command.ReplyToCommand($"[LoadMatchFromURL] HTTP 请求失败 code: {response.StatusCode}");
        ...
        public void HandleTeamNameChangeCommand(CCSPlayerController? player, string teamName, int teamNum) {
            ...
            if (matchStarted) {
                ReplyToUserCommand(player, "比赛已经开始队名不能更改!");
                return;
            }
            teamName = RemoveSpecialCharacters(teamName.Trim());
            if (teamName == "") {
                ReplyToUserCommand(player, $"请使用: !team{teamNum} <name>");
            }
