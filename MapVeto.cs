    ...
    public partial class MatchZy
    {
        ...
        public void VetoCountdown()
        {
            ...
            if (warningsPrinted >= vetoCountdownTime)
            {
                ...
                Server.PrintToChatAll($"{chatPrefix}  {ChatColors.Green}{matchzyTeam1.teamName} 的队长是{ChatColors.Default}: {ChatColors.Green}{playerData[team1Captain].PlayerName}{ChatColors.Default}");
                Server.PrintToChatAll($"{chatPrefix}  {ChatColors.Green}{matchzyTeam2.teamName} 的队长是{ChatColors.Default}: {ChatColors.Red}{playerData[team2Captain].PlayerName}{ChatColors.Default}");
            ...
            Server.PrintToChatAll($"{chatPrefix} 离选择地图还剩 {ChatColors.Red}{secondsRemaining} 秒");
        }
        ...
        public void PromptForMapSelectionInChat(string option) {
            ...
            switch (option) 
            {
                case "team1_ban":
                    ...
                    stepMessage = $"请使用 .ban <地图名> 来禁用一张地图.";
                    break;
                case "team2_ban":
                    ...
                    break;                                                       
                case "team1_pick":
                    action = $"{ChatColors.Green}{matchzyTeam1.teamName}{ChatColors.Default} 现在必须 {ChatColors.Green}选择{ChatColors.Default} 一张地图 {matchConfig.Maplist.Count + 1}.";
                    client = vetoCaptains["team1"];
                    stepMessage = $"请使用 .pick <地图名> 来选择一张地图.";
                    break;
                case "team2_pick":
                    action = $"{ChatColors.Green}{matchzyTeam2.teamName}{ChatColors.Default} 现在必须 {ChatColors.Green}选择{ChatColors.Default} 一张地图 {matchConfig.Maplist.Count + 1}.";
                    client = vetoCaptains["team2"];
                    stepMessage = $"请使用 .pick <地图名> 来选择一张地图.";
                    break;
            }
        ...
        public void HandeMapBanCommand(CCSPlayerController player, string map)
        {
            ...
            if (!BanMap(map, playerTeam)) {
                player.PrintToChat($"{chatPrefix} {map} 不是一个有效的地图.");
            } else {
                HandleVetoStep();
            }
        }
        ...
        public void HandeMapPickCommand(CCSPlayerController player, string map)
        {
            ...
            if (!PickMap(map, playerTeam)) {
                player.PrintToChat($"{chatPrefix} {map} 是无效的地图.");
            } else {
                HandleVetoStep();
            }
        }
        ...
        public bool PickMap(string mapName, int team) {
            ...
            if (team != 0) {
                ...
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{matchzyTeam.teamName}{ChatColors.Default} 选择了 {ChatColors.Green}{mapRemovedName}{ChatColors.Default}  {matchConfig.Maplist.Count + 1}");
            }
        ...
        public void AbortVeto()
        {
            ...
            Server.PrintToChatAll($"{chatPrefix} 一个队伍的队长离开,地图暂停选择。");
            Server.PrintToChatAll($"{chatPrefix} 当您准备好继续选择地图时,输入 .ready ");
            isPreVeto = true;
            isVeto = false;
            if (isPaused)
            {
        ...
        {
            Server.PrintToChatAll($"{chatPrefix} 这些地图已确定:");
        ...
        public void PromptForSideSelectionInChat(CsTeam team) {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{matchzyTeam.teamName}{ChatColors.Default} 现在必须选择一边来游玩 地图名:{ChatColors.Green}{mapName}{ChatColors.Default}");

            int client = vetoCaptains[teamString];
            if (!playerData.ContainsKey(client) || !playerData[client].IsValid) return;

            playerData[client].PrintToChat($"{chatPrefix} 使用 {ChatColors.Green}.ct {ChatColors.Default}或者 {ChatColors.Green}.t {ChatColors.Default}来选边");
        }
        ...
        public void PickSide(CsTeam side, string team) {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{matchzyTeam.teamName}{ChatColors.Default} 选择以 {ChatColors.Green}{sideFormatted}{ChatColors.Default} 在地图: {ChatColors.Green}{mapName}{ChatColors.Default}.");
