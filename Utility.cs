    ...
    public partial class MatchZy
    {
        ...
        private void SendUnreadyPlayersMessage() {
            if (isWarmup && !matchStarted) {
                ...
                if (unreadyPlayers.Count > 0) {
                    ...
                    string minimumReadyRequiredMessage = isMatchSetup ? "" : $"[至少要有 {ChatColors.Green}{minimumReadyRequired}{ChatColors.Default} 名玩家准备才能开始比赛！]";

                    Server.PrintToChatAll($"{chatPrefix} 未准备的玩家: {unreadyPlayerList}. 请在聊天框输入 .ready 以准备! {minimumReadyRequiredMessage}");
                } else {
                    int countOfReadyPlayers = playerReadyStatus.Count(kv => kv.Value == true);
                    if (isMatchSetup)
                    {
                        Server.PrintToChatAll($"{chatPrefix} 当前已准备的玩家: {ChatColors.Green}{countOfReadyPlayers}{ChatColors.Default}名");
                    }
                    else
                    {
                        Server.PrintToChatAll($"{chatPrefix} 至少要 {ChatColors.Green}{minimumReadyRequired}{ChatColors.Default} 名玩家准备, 当前已准备的玩家: {ChatColors.Green}{countOfReadyPlayers}{ChatColors.Default}名");
                    }
                }
            }
        }
        ...
        private void SendPausedStateMessage() {
            if (isPaused && matchStarted) {
                ...
                if ((string)pauseTeamName == "Admin") {
                    Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}Admin{ChatColors.Default} 暂停了比赛.");
                } else if ((string)pauseTeamName == "RoundRestore" && !(bool)unpauseData["t"] && !(bool)unpauseData["ct"]) {
                    Server.PrintToChatAll($"{chatPrefix} 比赛因为回合回溯暂停. 双方需要输入 {ChatColors.Green}.unpause{ChatColors.Default} 来结束暂停！");
                } else if ((bool)unpauseData["t"] && !(bool)unpauseData["ct"]) {
                    Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{reverseTeamSides["TERRORIST"].teamName}{ChatColors.Default} 想要取消暂停。 {ChatColors.Green}{reverseTeamSides["CT"].teamName}{ChatColors.Default}, 请输入 {ChatColors.Green}!unpause {ChatColors.Default}取消暂停.");
                } else if (!(bool)unpauseData["t"] && (bool)unpauseData["ct"]) {
                    Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{reverseTeamSides["CT"].teamName}{ChatColors.Default} 想要取消暂停。 {ChatColors.Green}{reverseTeamSides["TERRORIST"].teamName}{ChatColors.Default}, 请输入 {ChatColors.Green}!unpause {ChatColors.Default}取消暂停.");
                } else if (!(bool)unpauseData["t"] && !(bool)unpauseData["ct"]) {
                    Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{pauseTeamName}{ChatColors.Default} 暂停了比赛！输入 {ChatColors.Green}.unpause {ChatColors.Default}取消暂停");
                }
            }
        }
        ...
        private void SendSideSelectionMessage() {
            if (isSideSelectionPhase) {
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{knifeWinnerName}{ChatColors.Default} 赢下刀局. 输入 {ChatColors.Green}.stay{ChatColors.Default} 不选边或者输入 {ChatColors.Green}.switch{ChatColors.Default}");
            }
        }
        ...
        private void StartAfterKnifeWarmup() {
            ...
            ShowDamageInfo();
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{knifeWinnerName}{ChatColors.Default} 赢下刀局. 输入 {ChatColors.Green}.stay{ChatColors.Default} 不选边或者输入 {ChatColors.Green}.switch{ChatColors.Default}换边");
        ...
        private void StartLive() {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}比赛开始!");
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}比赛开始!");
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}比赛开始!");
        ...
        private void HandleMapChangeCommand(CCSPlayerController? player, string mapName) {
            ...
            if (matchStarted) {
                ReplyToUserCommand(player, $"当比赛开始时不可以更换地图!");
                return;
            }
        ...
        private void HandleReadyRequiredCommand(CCSPlayerController? player, string commandArg) {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg)) {
                if (int.TryParse(commandArg, out int readyRequired) && readyRequired >= 0 && readyRequired <= 32) {
                    ...
                    ReplyToUserCommand(player, $"开始比赛所需的最少准备玩家数现在设置为: {minimumReadyRequiredFormatted}");
                    CheckLiveRequired();
                }
                else {
                    ReplyToUserCommand(player, $"无效的数值。请指定一个有效的非负数。 请使用: !readyrequired <所需的最少准备玩家数>");
                }
            }
            else {
                string minimumReadyRequiredFormatted = (player == null) ? $"{minimumReadyRequired}" : $"{ChatColors.Green}{minimumReadyRequired}{ChatColors.Default}";
                ReplyToUserCommand(player, $"当前至少需要: {minimumReadyRequiredFormatted} 准备. 使用: !readyrequired <所需的最少准备玩家数>");
            }
        }
        ...
        private void HandleMatchStart() {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}MatchZy V0.7汉化版{ChatColors.Default} 插件 by {ChatColors.Green}L1mit1337(https://github.com/L1mit1337/){ChatColors.Default}");
        }
        ...
        private void HandleMatchEnd() {
            ...
            if (matchzyTeam1.seriesScore > matchzyTeam2.seriesScore) {
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{matchzyTeam1.teamName}{ChatColors.Default} 获胜！ 比分:{ChatColors.Green}{matchzyTeam1.seriesScore}-{matchzyTeam2.seriesScore}{ChatColors.Default}");

            } else if (matchzyTeam2.seriesScore > matchzyTeam1.seriesScore) {
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{matchzyTeam2.teamName}{ChatColors.Default} 获胜！ 比分:{ChatColors.Green}{matchzyTeam2.seriesScore}-{matchzyTeam1.seriesScore}{ChatColors.Default}");

            } else {
                Server.PrintToChatAll($"{chatPrefix} 平局！ 比分: {ChatColors.Green}{matchzyTeam1.seriesScore}-{matchzyTeam2.seriesScore}{ChatColors.Default}");
            }
            ...
            try {
                if (isMatchLive) {
                    ...
                    if (stopData["ct"] && !stopData["t"]) {
                        Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{reverseTeamSides["CT"].teamName}{ChatColors.Default} 的回合回溯请求在回合结束时被取消");
                    } else if (!stopData["ct"] && stopData["t"]) {
                        Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{reverseTeamSides["TERRORIST"].teamName}{ChatColors.Default} 的回合回溯请求在回合结束时被取消");
                    }
        ...
        private void ReplyToUserCommand(CCSPlayerController? player, string message, bool console = false)
        {
            if (player == null) {
                ...
                if (console) {
                    player.PrintToConsole($"[MatchZy v0.7汉化版] {message}");
                } else {
                    player.PrintToChat($"{chatPrefix} {message}");
                }
            }
        ...
        private void PauseMatch(CCSPlayerController? player, CommandInfo? command) {
            if (isMatchLive && isPaused) {
                ReplyToUserCommand(player, "比赛已经暂停!");
                return;
            }
            if (IsHalfTimePhase())
            {
                ReplyToUserCommand(player, "你不能使用这个指令在半场休息中.");
                return;
            }
            if (IsPostGamePhase())
            {
                ReplyToUserCommand(player, "你不能使用这个指令在游戏结束时.");
                return;
            }
            if (IsTacticalTimeoutActive())
            {
                ReplyToUserCommand(player, "当技术暂停时你不能使用这个指令.");
                return;
            }
            if (isMatchLive && !isPaused) {
                ...
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{pauseTeamName}{ChatColors.Default} 暂停了比赛. 输入 .unpause 结束暂停");
        ...
        private void ForcePauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (player == null) {
                Server.PrintToConsole($"[MatchZy] 管理员暂停了比赛.");
            } 
            SetMatchPausedFlags();
        }
        ...
        private void ForceUnpauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            if (isMatchLive && isPaused) {
                ...
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}管理员{ChatColors.Default} 取消了暂停, 比赛继续!");
                UnpauseMatch();

                if (player == null) {
                    Server.PrintToConsole("[MatchZy] 管理员取消了暂停，比赛继续!");
                }
            }
        }
        ...
        private void StartMatchMode() 
        {
            ...
            ExecUnpracCommands();
            ResetMatch();
            Server.PrintToChatAll($"{chatPrefix} 比赛模式加载完成!");
        }
        ...
        private void SendPlayerNotAdminMessage(CCSPlayerController? player) {
            ReplyToUserCommand(player, "你没有权限使用这个指令!");
        }
        ...
        private void SendAvailableCommandsMessage(CCSPlayerController? player)
        {
            if (isPractice)
            {
                ReplyToUserCommand(player, "可以使用的指令: .spawn, .ctspawn, .tspawn, .bot, .nobots, .god, .clear, .fastforward, .dryrun");
            ...
            if (readyAvailable)
            {
                ReplyToUserCommand(player, "可以使用的指令: !ready, !unready");
                return;
            }
            if (isSideSelectionPhase)
            {
                ReplyToUserCommand(player, "可以使用的指令: !stay, !switch");
                return;
            }
            if (matchStarted)
            {
                string stopCommandMessage = isStopCommandAvailable ? ", !stop" : "";
                ReplyToUserCommand(player, $"可以使用的指令: !pause, !unpause, !tac, !tech{stopCommandMessage}");
                return;
            }
        }
