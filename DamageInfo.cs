                ...
                foreach (var entry in playerDamageInfo)
                {
                    ...
                    foreach (var (targetId, targetEntry) in entry.Value)
                    {
                        ...
                        if (attackerController != null && targetController != null)
                        {
                            ...
                            attackerController.PrintToChat($"{chatPrefix} {ChatColors.Green}造成{ChatColors.Default}: [{ChatColors.Red}{damageGiven} {ChatColors.Default}伤害/ {ChatColors.Lime}{hitsGiven}{ChatColors.Default} 次命中] {ChatColors.Orange}受到{ChatColors.Default}: [{ChatColors.Red}{damageTaken}{ChatColors.Default} 伤害/{ChatColors.Lime} {hitsTaken}{ChatColors.Default} 次命中] - {targetName} - ({ChatColors.Red}{targetHP} {ChatColors.Default}hp)");
                            targetController.PrintToChat($"{chatPrefix} {ChatColors.Green}造成{ChatColors.Default}: [{ChatColors.Red}{damageTaken} {ChatColors.Default}伤害/ {ChatColors.Lime}{hitsTaken}{ChatColors.Default} 次命中] {ChatColors.Orange}受到{ChatColors.Default}: [{ChatColors.Red}{damageGiven}{ChatColors.Default} 伤害/{ChatColors.Lime} {hitsGiven} {ChatColors.Default} 次命中] - {attackerName} - ({ChatColors.Red}{attackerHP} {ChatColors.Default}hp)");
                        }
