...
namespace MatchZy
{
    ...
    public partial class MatchZy : BasePlugin
    {
        ...
        public override string ModuleName => "MatchZyCN";
        public override string ModuleVersion => "0.1.1";

        public override string ModuleAuthor => "L1MIT1337- (https://gitee.com/L1mit1337/)";

        public override string ModuleDescription => "base on MatchZy v0.7.0(https://github.com/shobhit-pathak/MatchZy/) the Chinese translation version by L1mit1337(https://gitee.com/L1mit1337/)";

        public string chatPrefix = $"[{ChatColors.Green}MatchZy{ChatColors.Default}]";
        public string adminChatPrefix = $"[{ChatColors.Gold}管理员{ChatColors.Default}]";
           ...
           RegisterEventHandler<EventRoundEnd>((@event, info) => {
            ...
            RegisterEventHandler<EventPlayerHurt>((@event, info) =>
			{
                ...
                if (isPractice)
                {
                    if (victim.IsBot) {
                        ...
                        @event.Attacker.PrintToChat($"{chatPrefix} 对{victim.PlayerName}造成 {damage} 伤害,还剩({postDamageHealth} hp)");
                    }
                    return HookResult.Continue;
                }
            ...
            RegisterEventHandler<EventPlayerChat>((@event, info) => {
                ...
                if (originalMessage.StartsWith(".asay")) {
                    ...
                    if (IsPlayerAdmin(player, "css_asay", "@css/chat")) {
                        if (commandArg != "") {
                            ...
                            ReplyToUserCommand(player, "请使用: .asay <消息>");
                        }
                    } else {
                ...
                if (originalMessage.StartsWith(".rcon")) {
                    ...
                    if (IsPlayerAdmin(player, "css_rcon", "@css/rcon")) {
                        ...
                        ReplyToUserCommand(player, "指令发送成功!");
                    } else {
                        SendPlayerNotAdminMessage(player);
                    }
                }
            ...
            RegisterEventHandler<EventPlayerBlind>((@event, info) =>
            {
                ...
                if (@event.Attacker.IsValid && player.SteamID != @event.Attacker.SteamID)
                {
                    ...
                    @event.Attacker.PrintToChat($"{chatPrefix} {@event.Userid.PlayerName}吃闪 被白时间: {roundedBlindDuration} 秒");
                }
